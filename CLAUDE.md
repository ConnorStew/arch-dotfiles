...
`github.com/ConnorStew/arch-dotfiles` is public. Don't commit anything that
identifies the hardware or is specific to one machine — drive models, serial
numbers, firmware revisions, SMART readings, MAC addresses, keys, tokens.
Machine-local detail goes in `notes/local/`, which is gitignored.
...
- `hypr/` → `~/.config/hypr/`
  - One `hyprland.conf` shared by both hosts. `xwayland { force_zero_scaling = true }` fixes Steam rendering blurry on the laptop's fractional-scale eDP-1 (Steam is CEF/XWayland-only; XWayland has no `fractional-scale-v1`, so Hyprland would downscale = blur) — Steam renders 1x-crisp-but-small, accepted since `steam -forcedesktopscaling` no longer works (tested); the setting is a no-op on the desktop's integer-scale monitors. Discord's identical-looking blur was different: its `exec-once` bypasses the `discord/` stow package's `.desktop` override, so the Ozone/Wayland flags are duplicated on the `exec-once` line. See `workarounds/steam-blurry-fractional-scale.md` and `workarounds/discord-fuzzy-xwayland.md`
  ...
  - `$clipsync` (bound to `SUPER SHIFT V`) copies the X11 clipboard into the Wayland one via `xclip -o | wl-copy`. Hyprland's XWM only claims the Wayland selection while an **XWayland window holds focus**, so a copy made in an XWayland app (MTG Arena's deck export under Proton, verified on Hyprland 0.55.4) is frequently invisible to native Wayland apps — the content sits in the X clipboard only. `wl-clip-persist` does NOT fix this (it keeps Wayland content alive after the source app *exits*, a different problem) and was tried and reverted. See `workarounds/mtga-xwayland-clipboard.md`
...
- `smartd.yml` — copies `smartd.conf` → `/etc/`, `smartd-notify` → `/usr/local/bin/` (0755), enables + starts `smartd.service`. Config is a single `DEVICESCAN` directive with `-n standby,q` (don't spin up a sleeping disk to poll it) and scheduled self-tests. Test hours are **daytime on purpose**: smartd does not catch up on slots missed while the machine was off — it polls every 30 min and tests only if the clock matches right then — and neither host is left on overnight. Short test daily 19:00 (~2 min); long test Saturday 13:00, which will abort on shutdown if it doesn't finish. `-R 5! -R 187!` forces **raw**-value tracking for reallocated sectors and uncorrectable errors — smartd otherwise reports only normalised changes, which lag by ~11 retired blocks per point. The trailing `!` is load-bearing: without it the change is logged as informational and the `-M exec` warning path never runs, so no notification fires (confirmed by test — a raw 1976→1977 change with the normalised value static at 99 logged but stayed silent). Also copies `smartd.conf.d` → `/etc/conf.d/smartd` (read by the unit's `EnvironmentFile`) setting `--savestates=/var/lib/smartmontools/smartd.`, so baselines persist across reboots; without it smartd re-baselines every startup and can only spot changes within one session, useless on a machine powered off nightly. Config changes `notify: Restart smartd` (handler in `site.yml`). Note `/etc/conf.d/smartd` is pacman-owned, so expect a `.pacnew` on smartmontools upgrades. Alerts go through `-m <nomailer> -M exec` — `<nomailer>` is a literal smartd keyword meaning "run the script, send no email", used because **no MTA is installed** on this host, so a plain `-m <address>` would fail into a log nobody reads. The script logs via `logger` first (a desktop notification goes nowhere if nobody is logged in) then `runuser`s into connor's session bus for `notify-send`. On a host with a drive that already has logged errors, expect one alert on first start — smartd has no stored baseline and treats the existing count as new. See `notes/disk-health.md`.
- `timeshift.yml` — points Timeshift's `backup_device_uuid` at the filesystem mounted at `timeshift_backup_mount` (default `/home`), so snapshots stop landing on the root partition, where a couple of rsync snapshots are enough to fill it. Timeshift's config selects a **device, not a path** (there is no directory key — check `/etc/timeshift/default.json` for the full schema), and it always creates `<root-of-that-filesystem>/timeshift/{snapshots,snapshots-daily,…}`. So this moves the tree from `/timeshift/` to `/home/timeshift/`; the directory name is hardcoded and can't be tucked away as e.g. `/home/.timeshift`. The UUID is resolved at runtime from `ansible_facts.mounts`, **never hardcoded**: this repo is public and filesystem UUIDs are machine-specific. Reads `/etc/timeshift/timeshift.json`, patches the one key, and writes the whole file back, so unrelated settings (excludes, schedule, retention counts) survive. Guarded three ways — skipped entirely if the config doesn't exist yet (it's only written after Timeshift's first run), if the target mount isn't a separate filesystem, and if the UUID already matches. That last guard is load-bearing rather than cosmetic: Timeshift rewrites this file itself (`snapshot_size`, `snapshot_count`) in its own formatting, so an unconditional `copy` would reformat it and report `changed` on every single run. Note the task only redirects *future* snapshots — existing ones under `/timeshift` are not migrated and must be moved or deleted by hand.
...
- `immich.yml` — gated on `deploy_immich` (default false; true on the desktop only). Copies `immich.service` → `/etc/systemd/system/`, enables `docker.service`, then enables + starts `immich.service`. The unit is a `Type=oneshot`/`RemainAfterExit=yes` wrapper around `docker compose up -d`, with `WorkingDirectory` pointed at `system/files/immich/` **in this repo checkout** — unlike every other root file here, the compose file and `.env` are read from the repo rather than copied to `/etc`, because `.env` holds `DB_PASSWORD` and `UPLOAD_LOCATION`/`DB_DATA_LOCATION` paths and is therefore **gitignored** (`system/files/immich/.env` in `.gitignore`). `docker-compose.yml` is upstream's, unmodified. The task asserts the `.env` exists before touching systemd, so a fresh clone fails with a clear message instead of a cryptic compose error — recreate it from upstream's `example.env`. Imported after `packages.yml` since it needs `docker`/`docker-compose` (both in `packages_common`). Web UI on port 2283.
- `user-config.yml` — ssh perms (see `ssh/` above); deploys the `wallpaper-cycle.{service,timer}` user units (from `system/files/wallpaper-cycle/`) into `~/.config/systemd/user/`; and enables the user timers `arch-update.timer`, `wallpaper-cycle.timer`. The arch-update **tray** is deliberately not enabled here: `arch-update` 4.0.0 dropped `arch-update-tray.service` (the package now ships only `arch-update.{service,timer}` under `/usr/lib/systemd/user/`), leaving the tray as a `NoDisplay` stub at `/usr/share/applications/arch-update-tray.desktop`. It's started by `exec-once = arch-update --tray` in `hyprland.conf` instead, so the task only cleans up the upgrade's dangling `graphical-session.target.wants/arch-update-tray.service` enable symlink. Do **not** "fix" this with an `~/.config/autostart/` entry — Hyprland implements no XDG autostart and never activates `xdg-desktop-autostart.target` (verified `inactive`; systemd's generator does create `app-*@autostart.service` units, but nothing pulls them in), so anything dropped there silently never runs.
