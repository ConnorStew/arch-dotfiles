...
- `hypr/` → `~/.config/hypr/`, `~/.config/systemd/user/wallpaper-cycle.{service,timer}`
  ...
  - `wallpaper-cycle.timer` (systemd --user) runs `wallpaper-cycle.sh` every 30 min for the actual periodic cycling; `SUPER SHIFT W` also runs it manually (bound in `hyprland.conf`). The timer is *enabled* by Ansible (`tasks/user-config.yml`), but the unit files themselves ship in this stow package
...
- `user-config.yml` — ssh perms (see `ssh/` above) and enables the user timers/services `arch-update.timer`, `arch-update-tray.service`, `wallpaper-cycle.timer`.
