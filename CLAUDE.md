...
- `hypr/` → `~/.config/hypr/`, `~/.config/systemd/user/wallpaper-cycle.{service,timer}`
  ...
  - `hyprlock.conf` is styled to match the installed `sddm-astronaut-theme` (see `sddm/` below) — same background image (`/usr/share/sddm/themes/sddm-astronaut-theme/Backgrounds/jake_the_dog.png`), palette (`#242455` bg / `#d8d8ff` text / `#6c6caa` placeholder), and `Thunderman` font, referenced directly from the theme's installed path rather than duplicated into this repo
